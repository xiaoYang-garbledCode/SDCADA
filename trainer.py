...
class cross_domain_trainer(object):
   ...
   This class contain the main training functions for our AdAtime
    ...
    def __init__(self, args):
        ...
        self.teach_model = 'teach_Model'
        #self.Discriminator_model = 'Discriminator_ATT'
        self.Discriminator_model = 'Discriminator_AR'
        # Exp Description
    ...
    def sweep(self):
        ...
        sweep_id = wandb.sweep(sweep_config, project=self.sweep_project, entity='1747749798')
    ...
    def train(self):
        ...
        path = './MAX_ACC'
        scores_save_path_total = os.path.join(path, "total_scores.xlsx")
        empth_df = pd.DataFrame()
        empth_df.to_excel(scores_save_path_total, index=False, header=False)
        with pd.ExcelWriter(scores_save_path_total, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
            df_total = pd.DataFrame(columns=["acc", "f1", "src", "trg", "run"])
            for i in scenarios:
                src_id = i[0]
                trg_id = i[1]
                ...
                for run_id in range(self.num_runs):  # specify number of consecutive runs
                    ...
                    teach_Model = get_backbone_class(self.teach_model)
                    Discriminator_Model = get_backbone_class(self.Discriminator_model)
                    algorithm = algorithm_class(backbone_fe, teach_Model, Discriminator_Model, self.dataset_configs, self.hparams, self.device)
                    ...
                    for epoch in range(1, self.hparams["num_epochs"] + 1):
                        ...
                        self.logger.debug(f'-------------------------------------')
                    endTime = time.time()
                    print("trainning: end_time - start_time: ", endTime - startTime)
                    print("\n")
                    ...
                    # 在循环中逐步构建 DataFrame
                    row_data = [self.acc, self.f1, src_id, trg_id, run_id]
                    df_total = df_total.append(pd.Series(row_data, index=df_total.columns), ignore_index=True)
                    # df_total.to_excel(writer, sheet_name="Sheet1",index=False,header=False)
            df_total.to_excel(writer, index=False, header=False)
    ...
    def evaluate(self):
        ...
        end_time = time.time()
        print("testing: end_time - start_time: ", end_time - start_time)
        print("\n")
